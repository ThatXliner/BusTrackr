...
export async function createWorld(host:HTMLDivElement,onTick:(s:Snapshot)=>void,onError:(s:string)=>void){
 ...
 const [data,dem,corridor,detail]=await Promise.all(['/data/routes.json','/terrain/elevation.json','/data/corridor.json','/data/detail-zones.json'].map(async url=>{const r=await fetch(url);if(!r.ok)throw Error('Route data could not be loaded.');return r.json();})) as [Routes,Elevation,number[][][],DetailZones];
